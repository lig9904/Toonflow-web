...
const manual = useManualCreativeDraft<string, { projectId:number; scriptId:number; version:number; base:string; snapshot:{scriptPlan:string;storyboardTable:string} }>({
  label: "分镜表", initial: "", id: () => `production:${projectStore().project?.id}:${editorStore.episodesId}:storyboardTable`,
  ...
  load: async () => {
    ...
    return {value:snapshot.storyboardTable,meta:{projectId,scriptId,version:Number(data.planningVersion),base:snapshot.storyboardTable,snapshot}};
  },
  commit: async (value, meta) => {
    if (Number(projectStore().project?.id)!==meta.projectId || editorStore.episodesId!==meta.scriptId || storyboardTable.value!==meta.base) throw new Error("已保存内容或编辑对象已更新，草稿已保留；请重新核对后保存");
    const snapshot = {...meta.snapshot,storyboardTable:value};
    const response:any=await axios.post("/production/saveFlowData",{projectId:meta.projectId,episodesId:meta.scriptId,expectedPlanningVersion:meta.version,saveIntent:"manual",field:"storyboardTable",allowClear:!value.trim(),data:snapshot});
  ...
  onSaved: (saved) => { if(editorStore.episodesId===saved.meta.scriptId && Number(projectStore().project?.id)===saved.meta.projectId) { storyboardTable.value=saved.value; editorStore.planningVersion=saved.meta.version; } },
});
...
watch(()=>storyboardTable.value,()=>{void manual.observeSaved().catch(()=>undefined);});
